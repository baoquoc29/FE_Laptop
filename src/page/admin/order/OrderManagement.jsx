...
import { money, getLineTotal, getOrderAmounts, getShippingStatusText } from '../../../Utils/orderHelpers';
...
const OrderManagement = () => {
  ...
  const columns = [
    ...
    // Thêm cột tổng tiền vào cấu hình columns - thêm vào trước cột "Thao tác"
    {
      ...
      width: 150,
      render: (_, record) => {
        const { totalAmount } = getOrderAmounts(record);
        return (
  ...
  const exportInvoiceToPdf = (order) => {
    ...
    // Tính tổng tiền – dùng helper
    const finalY = (doc.lastAutoTable?.finalY || tableStartY + 5) + 15;
    const { subTotal: pdfSubTotal, discountAmount: pdfDiscount, shippingFee: pdfShipping, totalAmount: pdfTotal } = getOrderAmounts(order);
    ...
    const summaryBoxX = (210 - summaryBoxWidth) / 2;
    // Tăng chiều cao box tuỳ thuộc có giảm giá / phí ship không
    let summaryBoxHeight = 30;
    if (pdfDiscount > 0) summaryBoxHeight += 10;
    if (pdfShipping > 0) summaryBoxHeight += 10;
    ...
    doc.setFont('TimesNewRoman', 'normal');
    doc.setFontSize(10);
    doc.text('Tổng tiền hàng:', labelX, summaryY);
    doc.text(`${new Intl.NumberFormat('vi-VN', { 
      style: 'decimal', 
      maximumFractionDigits: 0
    }).format(pdfSubTotal)} VNĐ`, valueX, summaryY, { align: 'right' });
    ...
    if (pdfDiscount > 0) {
      ...
      doc.text(`-${new Intl.NumberFormat('vi-VN', { 
        style: 'decimal', 
        maximumFractionDigits: 0
      }).format(pdfDiscount)} VNĐ`, valueX, summaryY, { align: 'right' });
    }

    // Hiển thị phí vận chuyển nếu có
    if (pdfShipping > 0) {
      summaryY += 8;
      doc.text('Phí vận chuyển:', labelX, summaryY);
      doc.text(`${new Intl.NumberFormat('vi-VN', { 
        style: 'decimal', 
        maximumFractionDigits: 0
      }).format(pdfShipping)} VNĐ`, valueX, summaryY, { align: 'right' });
    }
    ...
    doc.text(`${new Intl.NumberFormat('vi-VN', { 
      style: 'decimal', 
      maximumFractionDigits: 0
    }).format(pdfTotal)} VNĐ`, valueX, summaryY, { align: 'right' });
            ...
            <>
              ...
              {currentOrder.orderItems && currentOrder.orderItems.length > 0 ? (
                <>
                  ...
                  {/* Hiển thị tổng tiền – dùng getOrderAmounts */}
                  {(() => {
                    const { subTotal: oSubTotal, discountAmount: oDiscount, shippingFee: oShipping, totalAmount: oTotal } = getOrderAmounts(currentOrder);
                    return (
                      <div style={{ 
                        display: 'flex', 
                        ...
                        >
                          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                            ...
                            <span style={{ fontWeight: 'bold' }}>{money(oSubTotal)}</span>
                          </div>
                          
                          {oDiscount > 0 && (
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                              <span>Giảm giá:</span>
                              <span style={{ fontWeight: 'bold', color: '#52c41a' }}>-{money(oDiscount)}</span>
                            </div>
                          )}
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                            ...
                            <span style={{ fontWeight: 'bold' }}>
                              {oShipping === 0 ? (
                                currentOrder.shippingProvider ? <span style={{ color: '#52c41a' }}>Miễn phí</span> : money(0)
                              ) : money(oShipping)}
                            </span>
                          </div>

                          {currentOrder.shippingProvider === 'GHN' && (
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                              <span>Đơn vị vận chuyển:</span>
                              <Tag color="orange">Giao Hàng Nhanh</Tag>
                            </div>
                          )}

                          {currentOrder.shippingOrderCode && (
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                              <span>Mã vận đơn:</span>
                              <span style={{ fontWeight: 'bold' }}>{currentOrder.shippingOrderCode}</span>
                            </div>
                          )}
                          ...
                          {currentOrder.shippingStatus && (
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                              ...
                              <Tag color="blue">{getShippingStatusText(currentOrder.shippingStatus)}</Tag>
                            </div>
                          )}
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                            ...
                            <span style={{ fontWeight: 'bold', fontSize: 18, color: '#ff4d4f' }}>{money(oTotal)}</span>
                          </div>
                        </Card>
                      </div>
                    );
            ...
            <div style={{ background: '#f5f5f5', padding: '12px', borderRadius: '4px', marginTop: '16px' }}>
              ...
              <p style={{ margin: '0 0 8px 0' }}>
                ...
                <span style={{ color: '#ff4d4f', fontWeight: 'bold' }}>
                  {money(getOrderAmounts(orderToRefund).totalAmount)}
                </span>
              </p>
            </div>
          )}
