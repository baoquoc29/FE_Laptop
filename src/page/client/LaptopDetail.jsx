                    ...
                    <div className="action-buttons">
                        ...
                        <div className="secondary-buttons">
                            ...
                            <Button
                                ...
                                type={isInCompare(productDetail?.productOptions[selectedOption]?.id) ? 'primary' : 'default'}
                                onClick={() => {
                                    const optId = productDetail?.productOptions[selectedOption]?.id;
                                    if (isInCompare(optId)) {
                                        removeFromCompare(optId);
                                        notification.info({ message: 'Đã xoá khỏi so sánh', placement: 'topRight' });
                                    } else {
                                        addToCompare({
                                            id: optId,
                                            name: productDetail?.product?.name,
                                            image: productDetail?.product?.images?.[0]?.url
                                        });
                                        notification.success({ message: 'Đã thêm vào so sánh', placement: 'topRight' });
                                    }
                                }}
                            >
                                <SwapOutlined />
                                {isInCompare(productDetail?.productOptions[selectedOption]?.id) ? 'Bỏ so sánh' : 'So sánh'}
                            </Button>
                        </div>
                    </div>
                </div>
